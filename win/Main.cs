...
public class MainApp {
 ...
 Box("Server").Text=profile.Server;Box("Community").Text=profile.Community;Box("Ip").Text=profile.Ip;Box("Mask").Text=profile.Mask;Box("Mac").Text=profile.Mac;Box("Mtu").Text=profile.Mtu.ToString();Get<ComboBox>("Cipher").ItemsSource=new[]{"AES-CBC","Twofish","ChaCha20","Speck-CTR"};Get<ComboBox>("Cipher").SelectedItem=profile.Cipher;Get<CheckBox>("Header").IsChecked=profile.Header;Get<ComboBox>("ConnectionMode").ItemsSource=new[]{"P2P 优先","强制服务器中转"};Get<ComboBox>("ConnectionMode").SelectedIndex=profile.Relay?1:0;Get<CheckBox>("Multicast").IsChecked=profile.Multicast;
 ...
 if(Get<ComboBox>("ConnectionMode").SelectedIndex<0)missing.Add("连接模式");
 if(missing.Count>0)throw new Exception("请填写："+string.Join("、",missing));
 profile.Server=Box("Server").Text.Trim();profile.Community=Box("Community").Text.Trim();profile.Ip=Box("Ip").Text.Trim();profile.Mask=Box("Mask").Text.Trim();profile.Mac=Box("Mac").Text.Trim();int mtu;if(!int.TryParse(Box("Mtu").Text,out mtu))throw new Exception("MTU 必须是整数");profile.Mtu=mtu;profile.Cipher=(string)Get<ComboBox>("Cipher").SelectedItem;profile.Header=Get<CheckBox>("Header").IsChecked==true;profile.Relay=Get<ComboBox>("ConnectionMode").SelectedIndex==1;profile.Multicast=Get<CheckBox>("Multicast").IsChecked==true;profile.Adapter=Get<ComboBox>("Adapter").SelectedItem is Tap?((Tap)Get<ComboBox>("Adapter").SelectedItem).Id:"";profile.Validate(Get<PasswordBox>("Key").Password);profile.Encrypt(Get<PasswordBox>("Key").Password);}
