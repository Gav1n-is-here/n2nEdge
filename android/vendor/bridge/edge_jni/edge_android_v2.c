...
#ifdef N2N_V3
/* n2n 3.0 exports these helpers but does not declare them in its public header. */
...
extern int fetch_and_eventually_process_data(n2n_edge_t *eee, SOCKET sock,
        uint8_t *pktbuf, uint16_t *expected, uint16_t *position, time_t now);
#endif
...
static n2n_verdict on_packet_from_peer(n2n_edge_t *eee, const n2n_sock_t *peer,
    ...
    if (*payload_size < UIP_LLH_LEN) return N2N_DROP;
    n2n_android_t *priv = (n2n_android_t *) edge_get_userdata(eee);
...
void on_main_loop_period(n2n_edge_t *eee, time_t now) {
    ...
    if (!priv->started_at) priv->started_at = now;
    if ((!priv->last_registration && now - priv->started_at > 20) ||
        (priv->last_registration && now - priv->last_registration > 60)) {
        pthread_mutex_lock(&g_status->mutex);
        int changed = g_status->running_status != EDGE_STAT_SUPERNODE_DISCONNECT;
        g_status->running_status = EDGE_STAT_SUPERNODE_DISCONNECT;
        pthread_mutex_unlock(&g_status->mutex);
        if (changed) g_status->report_edge_status();
    }
...
int start_edge_v3(n2n_edge_status_t *status) {
    ...
    keep_on_running = 1;
    /* Only a REGISTER_SUPER_ACK may publish CONNECTED. */
    traceEvent(TRACE_NORMAL, "edge started");    
