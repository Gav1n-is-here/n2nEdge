...
static n2n_edge_status_t status = {.mutex = PTHREAD_MUTEX_INITIALIZER};
...
#ifndef NDEBUG
            __android_log_write(ANDROID_LOG_DEBUG, "edge_jni", "encryption key configured");
#endif /* #ifndef NDEBUG */
        }
    }
    // encKeyFile
...
void ResetEdgeStatus(JNIEnv *env, uint8_t cleanup) {
    ...
    if (status.tid != -1 || cleanup) {
        ...
        if (status.cmd.devDesc) {
            free(status.cmd.devDesc);
        }
